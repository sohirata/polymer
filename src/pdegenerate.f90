...
!if (ktarget /= 25) cycle
       ...
       IF (LDONE) THEN
        ...
        EXIT
       ELSE
        CALL DCC_GEN(ORDER,KTARGET,REFA,REFB,NDEGEN,LGUESS)
       ENDIF
      ENDDO
     ENDDO
     DEALLOCATE(CFHALFA,ADDRSSA,NORDERA)
...
SUBROUTINE DCC_GEN(ORDER,KTARGET,ICFA,ICFB,NDEGEN,LGUESS)
...
SUBROUTINE THERMAL_T_UPDATE(ITER,NDEGEN,ICFA,ICFB,TFILE,EXPTFILE,HEXPTFILE,DELTAFILE,ORDER,CCE,CCS,DEVSQ1,DEVSQ2,&
                            REFOCCA,REFOCCB,REFVIRA,REFVIRB)
...
!write(6,'(a,1f20.10)') 'delta fresh',dev2
       DO M=1,NDEGEN
        IF (D_NORDA(M)+D_NORDB(M) <= ORDER) THEN
!if ((ia==4).and.(ib==9)) &
!write(6,'(a,i3,3f20.10)') 'delta subtraction',m,dev2,-VEC4(IB,IA)*CCE(M,N),DEV2-VEC4(IB,IA)*CCE(M,N)
         REWIND(EXPTFILE+M)
         READ(EXPTFILE+M) VEC4
         DEV2=DEV2-VEC4(IB,IA)*CCE(M,N)
!       ELSE
!        WRITE(6,'(2I3,A,I3,A,I3,A)') IA,IB,' IS ',D_NORDA(M)+D_NORDB(M),' EXCITATIONS FROM ',M,'-TH DEGENERATE STATE'
        ENDIF
...
!      IF ((E0(J)-PREVIOUSMIN > 1.0D-9).AND.(E0(J) < CURRENTMIN)) THEN
     ...
     ENDDO
     IF (L /= NCFA*NCFB) CALL PABORT('ERROR TRAP IN DEGENERATE_CI')
     !
     ! SOLVE DEGENERATE CI
     !
...
!write(*,*) 'mapping failure logic bypassed'
     IF (LEXIST) CALL PABORT('MAPPING FAILURE')
