...
!endif
   ...
   DO JTARGET=1,IOPTN(59)
    ...
    ALLOCATE(EHC(JDEGEN,JDEGEN,0:ORDER),VHC(NCFA*NCFB,JDEGEN,0:ORDER))
    ALLOCATE(EHCSUM(JDEGEN,JDEGEN))
    EHC=0.0D0
    DO I=1,JDEGEN
     EHC(I,I,0)=E0(LIST(I))
    ENDDO
...
!      ISTATE=(I-1)*NCFB+J
!      JTARGET=0
     DO KTARGET=1,NTARGETS
      ISTATE=ITARGETS(KTARGET)
      J=MOD(ISTATE-1,NCFB)+1
      I=(ISTATE-J)/NCFB+1
...
! PERFORM A DEGENERATE COUPLED-CLUSTER CALCULATION 
   ...
   CALL CPU_TIME(ICPUS)
   MAXORDER=MIN(2*(IALL(0,0,0)-IVIRTCORE)-NELEA-NELEB,NELEA+NELEB-2*ICORE)
...
!integer,parameter :: itarget=4034 ! excited state, 4-fold
   ...
   DOUBLE PRECISION :: D
   INTEGER,ALLOCATABLE :: INDX(:)
   DOUBLE PRECISION,ALLOCATABLE :: VEC1(:,:),VEC2(:,:),VEC3(:,:),VEC4(:,:,:)
   CHARACTER(3)  :: APPEND
   CHARACTER(15) :: FULLAPPEND
   INTEGER :: TORDER
...
!      IF ((E0(J)-PREVIOUSMIN > 1.0D-9).AND.(E0(J) < CURRENTMIN)) THEN
     ...
     DO I=1,NTARGETS
      ...
      MDEGEN(I)=K
      L=L+K
      WRITE(6,'(A,I3,A,I10,A,F15.10,I5,A,100I8:)') 'TARGET',I,' = STATE',ITARGETS(I),' ZEROTH-ORDER ENERGY = ',E0(ITARGETS(I)),&
     ...
     IF (LFULL) THEN
      ...
      WRITE(6,'(/,A,I8,A,I8,A)') 'FULL DIAGONALIZATION OF ',NCFA*NCFB,' X ',NCFA*NCFB,' MATRIX'
      ALLOCATE(EDCI(NCFA*NCFB,NCFA*NCFB))
      ALLOCATE(EI(NCFA*NCFB),VR(NCFA*NCFB,NCFA*NCFB),VL(1,NCFA*NCFB),WK(4*NCFA*NCFB))
      CALL DCI_GEN(0)
      ! DIAGONALIZE
      CALL DGEEV('N','V',NCFA*NCFB,EDCI,NCFA*NCFB,ER,EI,VL,1,VR,NCFA*NCFB,WK,4*NCFA*NCFB,INFO)
      IF (INFO /= 0) CALL PABORT('DGEEV FAILED TO DIAGONALIZE A MATRIX')
...
!       WRITE(*,'(I7,I6,I6,I12)') I,J,MLIST(J,I),REFMAP(MLIST(J,I))
     ...
     ELSE
      ...
      DO I=1,NTARGETS
       WRITE(6,'(/,A,I3)') 'TARGET',I
       DO J=1,MDEGEN(I)
        WRITE(FULLAPPEND,'(I2.2,A,I2.2,A,I5.5,A,I3.3)') NELEA,'.',NELEB,'.',I,'.',J
        OPEN(800+J,FILE=TRIM(COPTN(1))//'.exact.'//FULLAPPEND,FORM='UNFORMATTED')
        WRITE(6,'(A,A)') 'EXACT WAVE FUNCTION WILL BE WRITTEN TO ',TRIM(COPTN(1))//'.exact.'//FULLAPPEND
       ENDDO
...
!    -----------------
     ...
     DO ISTATE=1,NTARGETS
      ...
      DO I=1,NDEGEN
       ...
       OPEN(800+I,FILE=TRIM(COPTN(1))//'.exact.'//FULLAPPEND,FORM='UNFORMATTED')
       OPEN(900+I,FILE=TRIM(COPTN(1))//'.expTexact.'//APPEND,FORM='UNFORMATTED')
       WRITE(6,'(A,A)') 'EXACT WAVE FUNCTION WILL BE READ FROM ',TRIM(COPTN(1))//'.exact.'//FULLAPPEND
      ENDDO
      OPEN(90,FILE=TRIM(COPTN(1))//'.tmp1',FORM='UNFORMATTED')
!     -----------------------------------------------------------------
!     GENERATE FULL CC T-AMPLITUDES (Li & Paldus, JCP, 119, 5320, 2003)
!     -----------------------------------------------------------------
      ALLOCATE(VEC4(NCFB,NCFA,NDEGEN))
      IF (LFULL) THEN
       ! 
       ! FULL DIAGONALIZATION
       !
...
!       IF (.NOT.LUSED) WRITE(*,'(I5,F20.10,5X,128F10.4:)') J,ER(J)+NUCLEAR_REPULSION,(VR(LIST(L),J),L=1,NDEGEN)
      ...
      ELSE
       ...
       DO I=1,NDEGEN
        REWIND(800+I)
        READ(800+I) VEC4(:,:,I)
        ICFB(I)=MOD(LIST(I)-1,NCFB)+1
        ICFA(I)=(LIST(I)-ICFB(I))/NCFB+1
       ENDDO
!      WRITE(*,'(5X,100I20:)') (I,I=1,NDEGEN)
       WRITE(*,'(5X,100I20:)') (LIST(I),I=1,NDEGEN)
       WRITE(*,'(5X,100F20.10:)') (ER(LIST(I)),I=1,NDEGEN)
       J=0
       DO IA=1,NCFA
        DO IB=1,NCFB
         J=J+1
         DO I=1,NDEGEN
          IF (J==LIST(I)) WRITE(*,'(I5,100F20.4:)') J,(VEC4(IB,IA,K),K=1,NDEGEN)
         ENDDO
        ENDDO
       ENDDO
      ENDIF
...
!write(*,'(i3,128f10.5:)') i,(c(j),j=1,ndegen)
        ...
        DO J=1,NDEGEN
         ...
         ELSE
          DO IA=1,NCFA
           DO IB=1,NCFB
            VEC1(IB,IA)=VEC1(IB,IA)+C(J)*VEC4(IB,IA,J)
           ENDDO
          ENDDO
         ENDIF
        ENDDO
      ...
      TORDER=MIN(2*(IALL(0,0,0)-IVIRTCORE)-NELEA-NELEB,NELEA+NELEB-2*ICORE)
...
!write(*,*) 'virtual  beta  = ',(REFVIRB(I),I=1,IVIRB)
       ...
       DO ORDER=1,TORDER
        ...
        REWIND(100+N)
        WRITE(100+N) VEC2
        WRITE(FULLAPPEND,'(I2.2,A,I2.2,A,I5.5,A,I3.3)') NELEA,'.',NELEB,'.',ISTATE,'.',N
        WRITE(6,'(A,I2.2,A,A)') 'EXACT T',ORDER,'-AMPLITUDE WRITTEN TO ',TRIM(COPTN(1))//'.T.'//FULLAPPEND
...
!write(*,*) 'mapping failure logic bypassed'
     ...
     DEALLOCATE(ER) 
     IF (LFULL) DEALLOCATE(EI,VR,VL,WK)
     DEALLOCATE(CFHALFA,ADDRSSA,NORDERA)
     DEALLOCATE(CFHALFB,ADDRSSB,NORDERB)
...
! CALCULATE FULL CI MATRIX
   ...
   INTEGER :: IA,IB
   INTEGER :: I,J,K,L,M,NOPEN
   INTEGER,ALLOCATABLE :: N(:)
   REAL :: MEM,ICPUS,ICPUE
   LOGICAL :: LDONE(MAXFILE)
   DOUBLE PRECISION :: DEVSQ,DEVMAX,D,F,S2
   DOUBLE PRECISION :: SMALLH(MAXFILE,MAXFILE),LAMBDA(MAXFILE)
   ...
   NROOTS=NDEGEN
   WRITE(6,'(A,I3)') 'NUMBER OF ROOTS SOUGHT = ',NROOTS
   WRITE(6,'(A,100I8:)') 'REFERENCES = ',(LIST(I),I=1,NDEGEN)

   ALLOCATE(VEC1(NCFB,NCFA),VEC2(NCFB,NCFA),N(IALLMAX-IVIRTCORE))
   OPEN(50,FILE=TRIM(COPTN(1))//'.fi0',FORM='UNFORMATTED')
   OPEN(51,FILE=TRIM(COPTN(1))//'.fo0',FORM='UNFORMATTED')
   ...
   DO ICYCLE=1,MAXCYCLE
    ...
    DO I=1,NTRIALS
     IF (.NOT.LDONE(I)) THEN
      CALL THERMAL_HAMILTONIAN_PRODUCT(50,51,I-1,1.0D0)
      LDONE(I)=.TRUE.
     ENDIF
    ENDDO
...
!   CALL DUMP10(W1,NTRIALS,MAXFILE)
    ...
    DO K=1,NROOTS
     ...
     DO L=1,NTRIALS
      ...
      IF (K > 1) THEN
       DO M=1,K-1
        IF (MAP(M)==L) LTAKEN=.TRUE.
       ENDDO
      ENDIF
      IF ((.NOT.LTAKEN).AND.(W1(K,L)**2 > MAXC)) THEN
       MAXI=L
       MAXC=W1(K,L)**2
      ENDIF
     ENDDO
    ...
    ENDDO
    DO K=1,NROOTS
!    WRITE(6,'(I3,A,I3)') K,'-TH ROOT IS NOW MAPPED TO ',MAP(K)
    ENDDO
    ...
    DO K=1,NROOTS
     ...
     DO L=1,NTRIALS
      READ(51) VEC2
      VEC1=VEC1+VEC2*W1(L,MAP(K))
      READ(50) VEC2
      VEC1=VEC1-LAMBDA(MAP(K))*VEC2*W1(L,MAP(K))
     ENDDO
     DEVSQ=0.0D0
     DO IA=1,NCFA
      DO IB=1,NCFB
     ...
     ENE(K)=LAMBDA(MAP(K))+NUCLEAR_REPULSION
     IF (NROOTS > 1) THEN
      WRITE(6,'(A,I3,F15.10,F15.5,F20.10,F12.1)') ' ROOT ',K,DEVSQ,(ENE(K)-ENE(1))*EV,ENE(K),(ICPUE-ICPUS)/DFLOAT(NROOTS)
     ELSE
      WRITE(6,'(A,I3,F15.10,F15.5,F20.10,F12.1)') ' ITER ',ICYCLE,DEVSQ,(ENE(K)-ENE(1))*EV,ENE(K),(ICPUE-ICPUS)/DFLOAT(NROOTS)
     ENDIF
    ...
    IF (DEVMAX < DOPTN(67)) THEN
     ...
     REWIND(51)
     DO I=1,NROOTS
      WRITE(6,'(A,I3,F20.10,A)') 'ROOT ',I,ENE(I),' HARTREE'
      REWIND(50)
      VEC1=0.0D0
      DO L=1,NTRIALS
       READ(50) VEC2
       VEC1=VEC1+VEC2*W1(L,MAP(I))
      ENDDO
      D=0.0D0
      DO IA=1,NCFA
       DO IB=1,NCFB
