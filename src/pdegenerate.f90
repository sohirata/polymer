   ...
   DO JTARGET=1,IOPTN(59)
     ...
     DO IB=1,NCFB
      ...
      IF (I == J) THEN
       ...
       DO K=1,IALL(0,0,0)-IVIRTCORE
        ...
        ENDIF
       ENDDO
       WRITE(6,'(A,I3,A,I7,A,A,A,A,A,F15.10,A,128I7:)') 'TARGET',JTARGET,' STRINGS ',I,' (',TRIM(CBITSA),',',TRIM(CBITSB),')',&
            E0(I),' DEGENERATE STATES = ',(LIST(K),K=1,JDEGEN)
      ENDIF
     ENDDO
    ENDDO
    ...
    DO IORDER=1,ORDER
     ...
     DO J=1,JDEGEN
      ...
      ENDDO
     ENDDO
     IF (LCHECK) WRITE(*,'(A,I5,X,I2,A,100I5)') '***** STATE ',I,JDEGEN,'-FOLD DEGENERATE:',(LIST(J),J=1,JDEGEN)
!    IF (.NOT.LCHECK) WRITE(*,'(A,I5,X,I2,A,100I5)') '===== STATE ',I,JDEGEN,'-FOLD DEGENERATE, UNCOUPLED:',(LIST(J),J=1,JDEGEN)
     WRITE(6,'(A,I3,A)') 'ORDER:',IORDER,' ACCUMULATED HIRSCHFELDER-CERTAIN ENERGY MATRIX'
! *** NOTE: Diagonalization of incremental (e.g., second-order) H-C energy matrix gives
!           correct eigenvalues, but the order of states is scrambled. Summing over eigenvalues
!           in the order of their sizes will be incorrect. The same eigenvector for a higher
...
!if (itarget /= 0) write(*,*) 'debug mode ******************** for state ',itarget
   ...
   DO NELE=2*ICORE,2*(IALL(0,0,0)-IVIRTCORE)
    DO NELEA=ICORE,NELE
     ...
     DO I=1,NCFA
      DO J=1,NCFB
       ISTATE=NALL_CHECK+(I-1)*NCFB+J
       JTARGET=0
       IF (LALL) THEN
        JTARGET=ISTATE
       ELSE
        DO K=1,IOPTN(59)      
         IF (ITARGETS(K)==ISTATE) JTARGET=ISTATE
        ENDDO
       ENDIF
       IF (JTARGET==0) CYCLE
!if ((itarget /= 0).and.(istate /= itarget)) cycle
       WRITE(6,'(A,I5)') '***** STATE ',ISTATE
       CBITSA=""
       CBITSB=""
       DO K=1,IALL(0,0,0)-IVIRTCORE
...
!enddo
    ...
    ENDDO
    CALL CPU_TIME(ICPUE)
    WRITE(6,'(A,I3,A,F10.2)') 'No.ELECTRONS',NELE,' CPU/SEC =',ICPUE-ICPUS
    CALL PFLUSH(6)
    CALL CPU_TIME(ICPUS)
   ENDDO
...
SUBROUTINE DCC_GEN(ORDER,ICFA,ICFB,NDEGEN)
! PERFORM A DEGENERATE COUPLED-CLUSTER CALCULATION 
   ...
   CHARACTER(3)  :: APPEND
   INTEGER :: REFOCCA(IALL(0,0,0),MAXNDEGEN),REFOCCB(IALL(0,0,0),MAXNDEGEN),IOCCA,IVIRA,NORDA
   INTEGER :: REFVIRA(IALL(0,0,0),MAXNDEGEN),REFVIRB(IALL(0,0,0),MAXNDEGEN),IOCCB,IVIRB,NORDB
   ...
   DO N=1,NDEGEN
    ...
    DO K=1,IALL(0,0,0)-IVIRTCORE
     ...
     ENDIF
    ENDDO
    WRITE(6,'(A,I3,A,2(I3,A,A,A))') 'REFERENCE ',N,' IS ',ICFA(N),' (',TRIM(CBITSA),')',ICFB(N),' (',TRIM(CBITSB),')'
   ENDDO
...
!    CALL CLEAN(ICFA(N),ICFB(N),100+N,ORDER)
     WRITE(6,'(A)') 'READING INITIAL T-AMPLITUDES FROM FILE'
     REWIND(100+N)
     READ(100+N) VEC1
    ELSE
     VEC1=0.0D0
     WRITE(6,'(A)') 'ZERO INITIAL T-AMPLITUDES FROM FILE'
     REWIND(100+N)
     WRITE(100+N) VEC1
    ENDIF
   ENDDO
...
!enddo
    ...
    DO N=1,NDEGEN
     DO M=1,NDEGEN
      ...
      IF (CCO(M,N) > ORDER) THEN
       CCE(M,N)=0.0D0
       CCS(M,N)=0.0D0
       WRITE(6,'(A)') '**** WARNING: DEGENERATE-SPACE ENERGY/OVERLAP MATRIX BEYOND T EXCITATION RANK IS ERASED'
!      WRITE(6,'(A)') '**** CAUTION: DEGENERATE-SPACE ENERGY/OVERLAP MATRIX IS FORMED BEYOND T EXCITATION RANK'
      ENDIF
...
!   CALL DUMP5(CCE_PRINT,NDEGEN)
    ...
    ! UPDATE T AMPLITUDES
    CALL THERMAL_T_UPDATE(ITER,NDEGEN,ICFA,ICFB,100,200,300,400,ORDER,CCE,CCS,DEVSQ1,DEVSQ2)
...
!   IF (MAXDEV > 1.0D2) CALL PABORT('COUPLED-CLUSTER ITERATION FAILED TO CONVERGE')
    IF (MAXDEV > 1.0D2) THEN
     CALL WARNING('COUPLED-CLUSTER ITERATION FAILED TO CONVERGE')
     EXIT
    ENDIF
...
!   IF (ITER==MAXITER) CALL PABORT('COUPLED-CLUSTER ITERATION FAILED TO CONVERGE')
    IF (ITER==MAXITER) CALL WARNING('COUPLED-CLUSTER ITERATION FAILED TO CONVERGE')
   ENDDO
...
tmp=dev2
       DO M=1,NDEGEN
        ...
        DEV2=DEV2-VEC4(IB,IA)*CCE(M,N)
!if (norda+nordb==1) write(*,'(a,3f20.10)') '   - ',vec4(ib,ia),cce(m,n),VEC4(IB,IA)*CCE(M,N)
! debug!!!!!
!       DEV2=DEV2-VEC4(IB,IA)*CCE(N,M)
       ENDDO
       IF (DABS(DEV2) > D) THEN
        IC=IA
        ID=IB
...
if (itarget /= 0) write(*,*) 'debug mode ******************** for state ',itarget
   ...
   ! SOLVE DEGENERATE CI
   !
   ALLOCATE(EDCI(NALL,NALL))
   CALL CPU_TIME(ICPUS)
   NALL_CHECK=0
   DO NELE=2*ICORE,2*(IALL(0,0,0)-IVIRTCORE)
    DO NELEA=ICORE,NELE
     NELEB=NELE-NELEA
     WRITE(6,'(A,2I3)') 'No.ELECTRONS',NELEA,NELEB
     IF (NELEA > IALL(0,0,0)-IVIRTCORE) CYCLE
     IF (NELEB > IALL(0,0,0)-IVIRTCORE) CYCLE
     IF (NELEA < ICORE) CYCLE
     IF (NELEB < ICORE) CYCLE
     CALL THERMAL_GENERATE_CONFIGURATIONSA(NELEA,MIN(NELEA,IALL(0,0,0)-IVIRTCORE-NELEA))
     CALL THERMAL_GENERATE_CONFIGURATIONSB(NELEB,MIN(NELEB,IALL(0,0,0)-IVIRTCORE-NELEB))
if ((itarget==0).or.((NALL_CHECK+1 <= itarget).and.(itarget <= NALL_CHECK+NCFA*NCFB))) then
     CALL DCI_GEN(NALL_CHECK)
!------------------------------- DEGENERATE REFS
if (itarget /= 0) then
     NDEGEN=0
     LIST=0
     DO J=NALL_CHECK+1,NALL_CHECK+NCFA*NCFB
      IF (DABS(E0(J)-E0(itarget)) < 1.0D-9) THEN
       NDEGEN=NDEGEN+1
       LIST(NDEGEN)=J
      ENDIF
     ENDDO
     IF (NDEGEN > MAXNDEGEN) CALL PABORT('ERROR')
     DO I=1,NDEGEN
      IB=MOD(LIST(I)-NALL_CHECK,NCFB)
      IA=(LIST(I)-NALL_CHECK-MOD(LIST(I)-NALL_CHECK,NCFB))/NCFB+1
      IF (NALL_CHECK+(IA-1)*NCFB+IB /= LIST(I)) CALL PABORT('ERROR 311')
      CBITSA=""
      CBITSB=""
      DO J=1,IALL(0,0,0)-IVIRTCORE
       ...
       ENDIF
      ENDDO
      WRITE(6,'(I3,A,I5,A,2(I2,A,A,A))') I,'-TH DEGENERATE REF',LIST(I),':',IA,' (',TRIM(CBITSA),')',IB,' (',TRIM(CBITSB),')'
     ENDDO
     WRITE(6,'(I2,A,11X,128I10)') NDEGEN,'-FOLD DEGENERATE:',(LIST(J),J=1,NDEGEN)
endif
!------------------------------- DIAGONALIZATION
     ...
     DO J=1,NCFA*NCFB
      DO L=1,NCFA*NCFB
       A(L,J)=EDCI(NALL_CHECK+L,NALL_CHECK+J)
      ENDDO
     ENDDO
...
!    FIRST DETERMINE THE STATE WITH MAX OVERLAP WITH DEGENERATE REFS
     ...
     DO I=1,NDEGEN
      ...
      DO IA=1,NCFA
       DO IB=1,NCFB
        J=(IA-1)*NCFB+IB
        IF (VR(LIST(I)-NALL_CHECK,J)**2 > OVLP) THEN
         LUSED=.FALSE.
         IF (I >= 2) THEN
          DO L=1,I+1
     ...
     DO J=1,NCFA*NCFB
      LUSED=.FALSE.
      DO I=1,NDEGEN
       IF ((itarget/=0).AND.(J==IFCI(I))) THEN
        WRITE(*,'(I5,F20.10,I5,128F10.4:)') J,ER(J)+NUCLEAR_REPULSION,LIST(I),(VR(LIST(L)-NALL_CHECK,J),L=1,NDEGEN)
        LUSED=.TRUE.
       ENDIF
      ENDDO
      IF ((itarget/=0).AND.(.NOT.LUSED)) &
        WRITE(*,'(I5,F20.10,5X,128F10.4:)') J,ER(J)+NUCLEAR_REPULSION,(VR(LIST(L)-NALL_CHECK,J),L=1,NDEGEN)
      IF (itarget==0) WRITE(*,'(I5,F20.10)') J,ER(J)+NUCLEAR_REPULSION
     ENDDO
...
!    NEXT DETERMINE THE EXACT WAVE FUNCTION FOR EACH DEGENERATE REF
     ...
     DO I=1,NDEGEN
      WRITE(APPEND,'(I3)') I
      OPEN(100+I,FILE=TRIM(COPTN(1))//'.T.'//APPEND,FORM='UNFORMATTED')
     ...
     DO I=1,NDEGEN
      DO J=1,NDEGEN
       B(I,J)=VR(LIST(I)-NALL_CHECK,IFCI(J))
      ENDDO
     ENDDO
...
!    NEXT REVERSE-COMPUTE T AMPLITUDES FROM EXACT WAVE FUNCTION
     TORDER=2*(IALL(0,0,0)-IVIRTCORE)-NELEA-NELEB
