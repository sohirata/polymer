...
program runtrn
  ...
  implicit none
  integer i,j,l,m,li,mi,lcmax,dim2
...
! dr=(log(rmax)-log(rmin))/(nr-1)
  do i=1,nr
     ar(i)=(1.0d0+dcos(dfloat(i)*pi/dfloat(nr+1)))/(1.0d0-dcos(dfloat(i)*pi/dfloat(nr+1)))
  enddo
...
!   set up a test case
  ...
  enddo
  rr(1)=0.0
  rr(2)=-1.07925214409303
  rr(3)=-1.47461105578086
...
!       rhx=log(xx)
!       k=(rhx-rhomin)/dr+1
        rhx=dacos((xx-1.0d0)/(xx+1.0d0))*dfloat(nr+1)/pi
        k=int(rhx)
        k=min(k,nr-3)
        k=max(k,3)
!       dlt=rhx-log(ar(k))
        dlt=rhx-dacos((ar(k)-1.0d0)/(ar(k)+1.0d0))*dfloat(nr+1)/pi
!       dy=dlt/dr
        dy=dlt
