...
! PERFORM FULL AND TRUNCATED CI, HIGH-ORDER RECURSIVE MP, AND HIGH-ORDER CC CALCULATIONS.
   ...
   INTEGER,PARAMETER :: ISELF=1 ! 1: SELF-ENERGY ; 2: GREEN'S FUNCTION ; 3: IP ; 4: EA ; 5: ENERGY
   INTEGER :: I,J,K
   ...
   IF (IOPTN(97) > 0) THEN
    ...
    ELSE IF (IOPTN(104)==1) THEN
     WRITE(6,'(A)') '* ALGORITHM = RECURSION 1                                 *'
...
!   C=0.0D0
!   CALL FULL_MBGF(OMEGA,C,SELF(:,:,1),ISELF)
!   WRITE(6,'(A)') " EXACT SELF-ENERGY / GREEN'S FUNCTION"
...
write(*,*) "\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/"
   ENDIF
   ENDIF
   IF ((.FALSE.).AND.(.NOT.LOPTN(60)).AND.(.NOT.LOPTN(61))) THEN
    ! CI_EE
...
write(*,*) "/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\"
    ! ****************************************
    ! * MBPT WITH LAMBDA VARIATION TECHNIQUE *
    ! ****************************************
    ! Note added in July, 2016: Up to MP4 barely reproduced within a few micro H
    !                           using the finite-difference (not all point fitting)
    ! ALL POINT FITTING USING LU DECOMPOSITION
    ALLOCATE(A(NSCALES,NSCALES),AS(NSCALES,NSCALES),B(NSCALES),BS(NSCALES),INDX(NSCALES))
!   DO I=1,NSCALES
!    SCALEFACTOR=0.4D0*DFLOAT(I-NSCALES/2)/DFLOAT(NSCALES)
!    DO J=1,NSCALES
!     A(I,J)=SCALEFACTOR**(J-1)
!    ENDDO
!    CALL CI_GUESS(MIN(IOPTN(51),2*MIN(IOCC-ICORE,IALL(0,0,0)-IOCC-IVIRTCORE)),NROOTS,NTRIALS)
!    CALL HIGHORDER_CI_SCALE(MIN(IOPTN(51),2*MIN(IOCC-ICORE,IALL(0,0,0)-IOCC-IVIRTCORE)),NROOTS,NTRIALS, &
!                            SCALEFACTOR)
!    B(I)=SCALEFACTOR
!   ENDDO
!   DO I=1,NSCALES
!    WRITE(6,'(2F20.10)') 0.4D0*DFLOAT(I-NSCALES/2)/DFLOAT(NSCALES),B(I)
!   ENDDO
!   AS=A
!   BS=B
!   CALL LUDCMP(A,NSCALES,NSCALES,INDX,C)
!   CALL LUBKSB(A,NSCALES,NSCALES,INDX,B)
!   CALL MPROVE(AS,A,NSCALES,NSCALES,INDX,BS,B)
!   DO I=1,NSCALES
!    WRITE(6,'(I3,F20.10)') I,B(I)
!   ENDDO
    ! 5-POINT FINITE DIFFERENCE
    WRITE(6,'(A,E10.3,/)') 'DELTA LAMBDA =',1.0D0/DFLOAT(NSCALES)
    WRITE(6,'(A)') 'FIVE-POINT FORMULA'
    DO I=1,5
     B(I)=DFLOAT(I-3)/DFLOAT(NSCALES)
     CALL CI_GUESS(MIN(IOPTN(51),2*MIN(IOCC-ICORE,IALL(0,0,0)-IOCC-IVIRTCORE)),NROOTS,NTRIALS)
     CALL HIGHORDER_CI_SCALE(MIN(IOPTN(51),2*MIN(IOCC-ICORE,IALL(0,0,0)-IOCC-IVIRTCORE)),NROOTS,NTRIALS,B(I))
     WRITE(6,'(I2,F20.10)') I-3,B(I)
    ENDDO
    WRITE(6,'(A)') 'CONSTANT'
    WRITE(6,'(F20.10)') B(3)
    WRITE(6,'(A)') 'FIRST DERIVATIVE'
    WRITE(6,'(F20.10)') (2.0D0*B(1)-16.0D0*B(2)+16.0D0*B(4)-2.0D0*B(5))/24.0D0*DFLOAT(NSCALES)
    WRITE(6,'(A)') 'SECOND DERIVATIVE'
    WRITE(6,'(F20.10)') (-B(1)+16.0D0*B(2)-30.0D0*B(3)+16.0D0*B(4)-B(5))/24.0D0*(DFLOAT(NSCALES)**2)
    WRITE(6,'(A)') 'THIRD DERIVATIVE'
    WRITE(6,'(F20.10)') (-2.0D0*B(1)+4.0D0*B(2)-4.0D0*B(4)+2.0D0*B(5))/24.0D0*(DFLOAT(NSCALES)**3)
    WRITE(6,'(A)') 'FOURTH DERIVATIVE'
    WRITE(6,'(F20.10)') (B(1)-4.0D0*B(2)+6.0D0*B(3)-4.0D0*B(4)+B(5))/24.0D0*(DFLOAT(NSCALES)**4)
    ! 7-POINT FINITE DIFFERENCE
    WRITE(6,'(/,A)') 'SEVEN-POINT FORMULA'
    DO I=1,7
     B(I)=DFLOAT(I-4)/DFLOAT(NSCALES)
     CALL CI_GUESS(MIN(IOPTN(51),2*MIN(IOCC-ICORE,IALL(0,0,0)-IOCC-IVIRTCORE)),NROOTS,NTRIALS)
     CALL HIGHORDER_CI_SCALE(MIN(IOPTN(51),2*MIN(IOCC-ICORE,IALL(0,0,0)-IOCC-IVIRTCORE)),NROOTS,NTRIALS,B(I))
     WRITE(6,'(I2,F20.10)') I-4,B(I)
    ENDDO
    WRITE(6,'(A)') 'CONSTANT'
    WRITE(6,'(F20.10)') B(4)
    WRITE(6,'(A)') 'FIRST DERIVATIVE'
    WRITE(6,'(F20.10)') (-B(1)+9.0D0*B(2)-45.0D0*B(3)+45.0D0*B(5)-9.0D0*B(6)+B(7))/60.0D0*DFLOAT(NSCALES)
    WRITE(6,'(A)') 'SECOND DERIVATIVE'
    WRITE(6,'(F20.10)') (2.0D0*B(1)-27.0D0*B(2)+270.0D0*B(3)-490.0D0*B(4)+270.0D0*B(5)-27.0D0*B(6)+2.0D0*B(7)) &
                        /180.0D0*(DFLOAT(NSCALES)**2)/2.0D0
    WRITE(6,'(A)') 'THIRD DERIVATIVE'
    WRITE(6,'(F20.10)') (B(1)-8.0D0*B(2)+13.0D0*B(3)-13.0D0*B(5)+8.0D0*B(6)-B(7))/8.0D0*(DFLOAT(NSCALES)**3)/6.0D0
    WRITE(6,'(A)') 'FOURTH DERIVATIVE'
    WRITE(6,'(F20.10)') (-B(1)+12.0D0*B(2)-39.0D0*B(3)+56.0D0*B(4)-39.0D0*B(5)+12.0D0*B(6)-B(7)) &
                        /6.0D0*(DFLOAT(NSCALES)**4)/24.0D0
    WRITE(6,'(A)') 'FIFTH DERIVATIVE'
    WRITE(6,'(F20.10)') (-B(1)+4.0D0*B(2)-5.0D0*B(3)+5.0D0*B(5)-4.0D0*B(6)+B(7)) &
                        /2.0D0*(DFLOAT(NSCALES)**5)/120.0D0
    DEALLOCATE(A,AS,B,BS,INDX)
write(*,*) "\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/"
   ENDIF
   IF (LOPTN(60)) THEN
    ! CI_IP
    IF (IOPTN(51) >= MAX(IOPTN(50),1)) THEN
     DO I=MAX(IOPTN(50),1),MIN(IOPTN(51),MIN(IOCC-ICORE,IALL(0,0,0)-IOCC-IVIRTCORE)+MIN(IOCC-ICORE,IALL(0,0,0)-IOCC-IVIRTCORE+1))
      IF (IOPTN(9) == 0) CALL FULL_IP_HAMILTONIAN(I)
      NROOTS=IOPTN(56)
   ...
   IF (LOPTN(61)) THEN
    ...
    IF (IOPTN(51) >= MAX(IOPTN(50),1)) THEN
     DO I=MAX(IOPTN(50),1),MIN(IOPTN(51),MIN(IOCC-ICORE,IALL(0,0,0)-IOCC-IVIRTCORE)+MIN(IOCC-ICORE+1,IALL(0,0,0)-IOCC-IVIRTCORE))
      IF (IOPTN(9) == 0) CALL FULL_EA_HAMILTONIAN(I)
      NROOTS=IOPTN(56)
   ...
   IF (IOPTN(59) > 0) THEN
    ...
    IF (LOPTN(60)) THEN
     ...
     IF (IOPTN(57) <= IOPTN(58)) THEN
      DO I=IOPTN(57),IOPTN(58)
       IF (IOPTN(9) == 0) CALL FULL_IP_EFF_H(MIN(IOPTN(53),2*MIN(IOCC-ICORE,IALL(0,0,0)-IOCC-IVIRTCORE)),I)
       NROOTS=IOPTN(59)
       CALL CI_IP_GUESS(I,NROOTS,NTRIALS)
    ...
    IF (LOPTN(61)) THEN
     ...
     IF (IOPTN(57) <= IOPTN(58)) THEN
      DO I=IOPTN(57),IOPTN(58)
       IF (IOPTN(9) == 0) CALL FULL_EA_EFF_H(MIN(IOPTN(53),2*MIN(IOCC-ICORE,IALL(0,0,0)-IOCC-IVIRTCORE)),I)
       NROOTS=IOPTN(59)
       CALL CI_EA_GUESS(I,NROOTS,NTRIALS)
