...
! PERFORM FULL AND TRUNCATED CI, HIGH-ORDER RECURSIVE MP, AND HIGH-ORDER CC CALCULATIONS.
   ...
   INTEGER,PARAMETER :: ISCAN=100 ! NUMBER OF FREQ POINTS IN EACH SIDE OF OMEGA 
...
! ---------------------------------------------------------------
   IF (IOPTN(97) > 0) THEN
    ...
    IF (IOPTN(104)==-1) THEN
     ...
     ENDDO
     DO I=-ISCAN,ISCAN
      OMEGA2=OMEGA+DFLOAT(I)/(10000.0D0/DFLOAT(ISCAN))
      WRITE(6,'(/,A)') '************************'
...
!   -----------------
    ...
    WRITE(6,'(A)') " EXACT SELF-ENERGY+FOCK (EIGENVALUES)"
    DO I=1,IALL(0,0,0)-IVIRTCORE-ICORE
     WRITE(6,'(F20.12)') ER(I)
    ENDDO
