...
! FOCK MATRIX ASSYMMETRY (DEVIATION FROM THE HERMITIAN) AS AN ARGUMENT.
   ...
   DOUBLE PRECISION :: MAXE,MINE,N1,N2,F1,F2
   DOUBLE PRECISION :: ZEROTEMP
...
!  DETERMINE THE FERMI ENERGY
   IF (DOPTN(98) > 0.0D0) THEN
    WRITE(6,'(A)') 'FOCK MATRIX IS FORMED AT T=0K'
    ZEROTEMP=1.0D0
    MAXE=-1.0D99
    MINE=1.0D99
    DO IX=-KVCX,MAX(0,KVCX-1)
    ...
    F2=MAXE
    N1=1.0D0
    DO WHILE (DABS(N1) > 1.0D-10)
     N1=0.0D0
     DO IX=-KVCX,MAX(0,KVCX-1)
     DO IY=-KVCY,MAX(0,KVCY-1)
     DO IZ=-KVCZ,MAX(0,KVCZ-1)
      DO J=1,NCGS
       N1=N1+2.0D0/(DEXP((EPSILON(J,IX,IY,IZ)-F1)/BOLTZMANN/ZEROTEMP)+1.0D0)
      ENDDO
     ENDDO
     ENDDO
     ENDDO
     ...
     DO IZ=-KVCZ,MAX(0,KVCZ-1)
      DO J=1,NCGS
       N2=N2+2.0D0/(DEXP((EPSILON(J,IX,IY,IZ)-F2)/BOLTZMANN/ZEROTEMP)+1.0D0)
      ENDDO
     ENDDO
     ENDDO
     ENDDO
    ...
    IF (IOPTN(9) == 3) THEN
     ...
     DO IY=-KVCY,MAX(0,KVCY-1)
     DO IZ=-KVCZ,MAX(0,KVCZ-1)
      WRITE(6,'(A,100F10.5:)') 'EPSILON = ',(EPSILON(J,IX,IY,IZ),J=1,NCGS)
      WRITE(6,'(A,100F10.5:)') 'WEIGHT =  ',(2.0D0/(DEXP((EPSILON(J,IX,IY,IZ)-FERMI)/BOLTZMANN/ZEROTEMP)+1.0D0),J=1,NCGS)
     ENDDO
     ENDDO
     ENDDO
    ENDIF
...
!   CONSTRUCT A NEW DENSITY MATRIX
    IF (DOPTN(98) == 0.0D0) THEN
     ...
     ENDDO
    ELSE
     IF (MYID == 0) WRITE(6,'(A)') 'DENSITY MATRIX IS FORMED AT T=0K'
     DO J=1,NCGS
      DO K=1,NCGS
       WS(K,J)=DCMPLX(0.0D0,0.0D0)
       WC(K,J)=DCMPLX(0.0D0,0.0D0)
       DO L=1,NCGS
        WS(K,J)=WS(K,J)+2.0D0/(DEXP((EPSILON(L,IX,IY,IZ)-FERMI)/BOLTZMANN/ZEROTEMP)+1.0D0) &
                       *DCONJG(CO(K,L,IX,IY,IZ))*CO(J,L,IX,IY,IZ)
        WC(K,J)=WC(K,J)+2.0D0/(DEXP((EPSILON(L,IX,IY,IZ)-FERMI)/BOLTZMANN/ZEROTEMP)+1.0D0) &
                       *DCONJG(CO(K,L,IX,IY,IZ))*CO(J,L,IX,IY,IZ) &
...
! THIS SUBROUTINE PASSES THE NECESSARY DATA TO EISPACK SUBROUTINES.
   ...
   CALL EPAC2(NP,N,E,D,VR,IERR)
   IF (IERR /= 0) CALL PABORT('EIGENVECTORS NOT CONVERGED')
...
SUBROUTINE EPAC2(NM,N,D,E,Z,IERR)
