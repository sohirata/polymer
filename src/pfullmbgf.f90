...
! SEE EQ.(13) OF PICKUP & GOSCINSKI, MOL.PHYS. 26, 1013 (1973).
   ...
   IMPLICIT NONE
   INTEGER :: ISELF ! 1: SELF-ENERGY ; 2: GREEN'S FUNCTION ; 3: IP ; 4: EA ; 5 ENERGY ; -1: SELF-ENERGY DERIVATIVE 
   ...
   DOUBLE PRECISION,ALLOCATABLE :: VIP1(:,:),VIP2(:,:),VIP3(:,:),DVIP(:,:)
   DOUBLE PRECISION,ALLOCATABLE :: VEA1(:,:),VEA2(:,:),VEA3(:,:),DVEA(:,:)
   DOUBLE PRECISION,ALLOCATABLE :: GREEN(:,:)
   DOUBLE PRECISION,ALLOCATABLE :: DGREEN(:,:)
...
!  IF (MEM > DOPTN(28)*1000000.0) CALL PABORT('OUT OF MEMORY') 
   ...
   ALLOCATE(VIP1(IP_NCF,NCF),VIP2(IP_NCF,NCF),VIP3(IP_NCF,NCF),DVIP(IP_NCF,NCF))
   ALLOCATE(VEA1(EA_NCF,NCF),VEA2(EA_NCF,NCF),VEA3(EA_NCF,NCF),DVEA(EA_NCF,NCF))
   ALLOCATE(GREEN(IALL(0,0,0)-IVIRTCORE-ICORE,IALL(0,0,0)-IVIRTCORE-ICORE))
   ALLOCATE(DGREEN(IALL(0,0,0)-IVIRTCORE-ICORE,IALL(0,0,0)-IVIRTCORE-ICORE))
...
!   WRITE(6,'(A,I3,A,F20.10)') 'EA ',IC,' = ',EEA(IC)-EN0
   ...
   DO MOQ=ICORE+1, IALL(0,0,0)-IVIRTCORE
    ...
    DO IC=1,NCF*IP_NCF
     ...
     DVIP=DVIP-VIP3*X/(OMEGA-EN0+EIP(IC))**2
!    VIP2=VIP2+VIP3*X
    ENDDO
     ...
     DO IB=1,IP_NCF
      IF (.NOT.BTEST(IP_CFHALF(IB),MOP-1)) THEN
       ...
       DO IA=1,NCF
        ...
        DGREEN(MOP-ICORE,MOQ-ICORE)=DGREEN(MOP-ICORE,MOQ-ICORE)+SGN*DVIP(IB,IA)*VEC0(IC,IA) ! FULL NORMALIZATION
...
!write(*,*) '********* EA ONLY!'
    ...
    DO IC=1,NCF*EA_NCF
     ...
     DVEA=DVEA-VEA3*X/(OMEGA+EN0-EEA(IC))**2
!    VEA2=VEA2+VEA3*X
    ENDDO
!write(*,*) '********** No energy denom!'
     ...
     DO IB=1,EA_NCF
      IF (BTEST(EA_CFHALF(IB),MOP-1)) THEN
       ...
       DO IA=1,NCF
        ...
        DGREEN(MOQ-ICORE,MOP-ICORE)=DGREEN(MOQ-ICORE,MOP-ICORE)+SGN*DVEA(IB,IA)*VEC0(IC,IA) ! FULL NORMALIZATION
...
!                       MOP=ICORE+1,IALL(0,0,0)-IVIRTCORE)
   ...
   ELSE IF (ISELF==-1) THEN
    ...
    DO IA=1,IC
     DO IB=1,IC
      A(IA,IB)=0.0D0
      DO ID=1,IC
       A(IA,IB)=A(IA,IB)+B(IA,ID)*DGREEN(ID,IB)
      ENDDO
     ENDDO
    ENDDO
    DO IA=1,IC
     DO IB=1,IC
      BS(IA,IB)=0.0D0
      DO ID=1,IC
       BS(IA,IB)=BS(IA,IB)+A(IA,ID)*B(ID,IB)
      ENDDO
     ENDDO
    ENDDO
    DO IA=1,IC
     BS(IA,IA)=BS(IA,IA)+1.0D0
    ENDDO
    SELFENERGY=BS
    DEALLOCATE(A,B,AS,BS,INDX)
   ...
   DEALLOCATE(EIP,WIP,EEA,WEA,VEC0,VEC1,VIP1,VIP2,VIP3,DVIP,VEA1,VEA2,VEA3,DVEA,GREEN,DGREEN)
...
SUBROUTINE FULL_MBGF_ROOTS
! CONSTRUCT ONE-PARTICLE EXACT MBGF FOR A GIVEN BASIS SET AND DETERMINE ITS ALL ROOTS & RESIDUES.
! SEE EQ.(13) OF PICKUP & GOSCINSKI, MOL.PHYS. 26, 1013 (1973).
   ...
   DOUBLE PRECISION,ALLOCATABLE :: VIP1(:,:),VIP2(:,:),VIP3(:,:),DVIP(:,:)
   DOUBLE PRECISION,ALLOCATABLE :: VEA1(:,:),VEA2(:,:),VEA3(:,:),DVEA(:,:)
   DOUBLE PRECISION,ALLOCATABLE :: GREEN(:,:),IGREEN(:,:)
   DOUBLE PRECISION,ALLOCATABLE :: DGREEN(:,:),DGREEN2(:,:)
   DOUBLE PRECISION :: EN0,X,SGN
   DOUBLE PRECISION,ALLOCATABLE :: VL(:,:),VR(:,:),ER(:),EI(:),WK(:)
   INTEGER :: INFO
   DOUBLE PRECISION,ALLOCATABLE :: A(:,:),B(:,:),AS(:,:),BS(:,:)
   INTEGER,ALLOCATABLE :: INDX(:)
   DOUBLE PRECISION :: C
   INTEGER :: IROOT,IROOT2,IAVERAGE
   DOUBLE PRECISION :: RESIDUE,RESSUM,RESSUM2,GM,GM2
   DOUBLE PRECISION,ALLOCATABLE :: RESIDUES(:,:)
   ...
   ALLOCATE(VEA1(EA_NCF,NCF),VEA2(EA_NCF,NCF),VEA3(EA_NCF,NCF),DVEA(EA_NCF,NCF))
   ALLOCATE(GREEN(IALL(0,0,0)-IVIRTCORE-ICORE,IALL(0,0,0)-IVIRTCORE-ICORE))
   ALLOCATE(IGREEN(IALL(0,0,0)-IVIRTCORE-ICORE,IALL(0,0,0)-IVIRTCORE-ICORE))
   ALLOCATE(DGREEN(IALL(0,0,0)-IVIRTCORE-ICORE,IALL(0,0,0)-IVIRTCORE-ICORE))
   ALLOCATE(DGREEN2(IALL(0,0,0)-IVIRTCORE-ICORE,IALL(0,0,0)-IVIRTCORE-ICORE))
   ALLOCATE(VL(1,IALL(0,0,0)-IVIRTCORE-ICORE),ER(IALL(0,0,0)-IVIRTCORE-ICORE),EI(IALL(0,0,0)-IVIRTCORE-ICORE))
   ALLOCATE(VR(IALL(0,0,0)-IVIRTCORE-ICORE,IALL(0,0,0)-IVIRTCORE-ICORE),WK(4*IALL(0,0,0)-IVIRTCORE-ICORE))
   ALLOCATE(A(IALL(0,0,0)-IVIRTCORE-ICORE,IALL(0,0,0)-IVIRTCORE-ICORE),B(IALL(0,0,0)-IVIRTCORE-ICORE,IALL(0,0,0)-IVIRTCORE-ICORE))
   ALLOCATE(AS(IALL(0,0,0)-IVIRTCORE-ICORE,IALL(0,0,0)-IVIRTCORE-ICORE),BS(IALL(0,0,0)-IVIRTCORE-ICORE,IALL(0,0,0)-IVIRTCORE-ICORE))
   ALLOCATE(INDX(IALL(0,0,0)-IVIRTCORE-ICORE))
   ALLOCATE(RESIDUES(IALL(0,0,0)-IVIRTCORE-ICORE,IALL(0,0,0)-IVIRTCORE-ICORE))
...
!   WRITE(6,'(A,I3,A,F20.10)') 'EA ',IC,' = ',EEA(IC)-EN0
   ...
   GM=0.0D0
!  =====================
   DO IROOT=1,NCF*IP_NCF+NCF*EA_NCF
!  =====================

   IF (IROOT <= NCF*IP_NCF) THEN
    OMEGA=EN0-EIP(IROOT)
   ELSE
    OMEGA=EEA(IROOT-NCF*IP_NCF)-EN0
   ENDIF
   ...
   DO MOQ=ICORE+1, IALL(0,0,0)-IVIRTCORE
    ...
    DO IC=1,NCF*IP_NCF
     ...
     DO IA=1,NCF
      ...
      ENDDO
     ENDDO
     VIP2=VIP2+VIP3*X/(OMEGA2-EN0+EIP(IC))
     DVIP=DVIP-VIP3*X/(OMEGA2-EN0+EIP(IC))**2
!    VIP2=VIP2+VIP3*X
    ENDDO
!write(*,*) '********** No energy denom!'
    ...
    DO MOP=ICORE+1, IALL(0,0,0)-IVIRTCORE
     DO IB=1,IP_NCF
      IF (.NOT.BTEST(IP_CFHALF(IB),MOP-1)) THEN
       ...
       DO IA=1,NCF
        GREEN(MOP-ICORE,MOQ-ICORE)=GREEN(MOP-ICORE,MOQ-ICORE)+SGN*VIP2(IB,IA)*VEC0(IC,IA) ! FULL NORMALIZATION
        DGREEN2(MOP-ICORE,MOQ-ICORE)=DGREEN2(MOP-ICORE,MOQ-ICORE)+SGN*DVIP(IB,IA)*VEC0(IC,IA) ! FULL NORMALIZATION
!       IF ((IC == 1).AND.(IA == 1)) GREEN(MOP,MOQ)=GREEN(MOP,MOQ)+SGN*VIP2(IB,IA) ! INTERMEDIATE NORMALIZATION
       ENDDO
      ENDIF
     ENDDO
...
!write(*,*) '********* EA ONLY!'
    ...
    DO IC=1,NCF*EA_NCF
     ...
     DO IA=1,NCF
      ...
      ENDDO
     ENDDO
     VEA2=VEA2+VEA3*X/(OMEGA2+EN0-EEA(IC))
     DVEA=DVEA-VEA3*X/(OMEGA2+EN0-EEA(IC))**2
!    VEA2=VEA2+VEA3*X
    ENDDO
!write(*,*) '********** No energy denom!'
    ...
    DO MOP=ICORE+1, IALL(0,0,0)-IVIRTCORE
     DO IB=1,EA_NCF
      IF (BTEST(EA_CFHALF(IB),MOP-1)) THEN
       ...
       DO IA=1,NCF
        GREEN(MOQ-ICORE,MOP-ICORE)=GREEN(MOQ-ICORE,MOP-ICORE)+SGN*VEA2(IB,IA)*VEC0(IC,IA) ! FULL NORMALIZATION
        DGREEN2(MOQ-ICORE,MOP-ICORE)=DGREEN2(MOQ-ICORE,MOP-ICORE)+SGN*DVEA(IB,IA)*VEC0(IC,IA) ! FULL NORMALIZATION
!       IF ((IC == 1).AND.(IA == 1)) GREEN(MOQ,MOP)=GREEN(MOQ,MOP)+SGN*VEA2(IB,IA)*VEC0(IC,IA) ! INTERMEDIATE NORMALIZATION
       ENDDO
      ENDIF
     ENDDO
...
!  INVERT GREEN'S FUNCTION
   ...
   DO IA=1,IC
    ...
    CALL MPROVE(AS,A,IC,IC,INDX,BS(:,IA),B(:,IA))
   ENDDO
!  IF (IAVERAGE==1) WRITE(6,'(A)') "INVERSE OF EXACT GREEN'S FUNCTION AT OMEGA-0.001"
!  IF (IAVERAGE==2) WRITE(6,'(A)') "INVERSE OF EXACT GREEN'S FUNCTION AT OMEGA+0.001"
!  CALL DUMP5(B,IALL(0,0,0)-IVIRTCORE-ICORE)
   IGREEN=IGREEN+B/2.0D0

   DO IA=1,IC
    DO IB=1,IC
     A(IA,IB)=0.0D0
     DO ID=1,IC
      A(IA,IB)=A(IA,IB)+B(IA,ID)*DGREEN2(ID,IB)
     ENDDO
    ENDDO
   ENDDO
   DO IA=1,IC
   ...
   DO IA=1,IC
    BS(IA,IA)=BS(IA,IA)+1.0D0
   ENDDO
!  IF (IAVERAGE==1) WRITE(6,'(A)') "EXACT SELF-ENERGY DERIVATIVE AT OMEGA-0.001"
!  IF (IAVERAGE==2) WRITE(6,'(A)') "EXACT SELF-ENERGY DERIVATIVE AT OMEGA+0.001"
!  CALL DUMP5(BS,IALL(0,0,0)-IVIRTCORE-ICORE)
   DGREEN=DGREEN+BS/2.0D0

   ENDDO ! LOOP OVER IAVERAGE

!  WRITE(6,'(A)') "INVERSE OF EXACT GREEN'S FUNCTION AT OMEGA"
!  CALL DUMP5(IGREEN,IALL(0,0,0)-IVIRTCORE-ICORE)
!  WRITE(6,'(A)') "EXACT SELF-ENERGY DERIVATIVE AT OMEGA"
!  CALL DUMP5(DGREEN,IALL(0,0,0)-IVIRTCORE-ICORE)
...
!  -----------------------
   ...
   DO IA=1,IC
    BS(IA,IA)=OMEGA
    DO IB=1,IC
     BS(IA,IB)=BS(IA,IB)-IGREEN(IA,IB)
    ENDDO
   ENDDO
!  WRITE(6,'(A)') "EXACT SELF-ENERGY+FOCK"
!  CALL DUMP5(BS,IALL(0,0,0)-IVIRTCORE-ICORE)
   A=BS
   CALL DGEEV('N','V',IC,A,IC,ER,EI,VL,1,VR,IC,WK,4*(IC),INFO)
   IF (INFO /= 0) CALL PABORT('DGEEV FAILED TO DIAGONALIZE A MATRIX')
   CALL PIKSRT(IALL(0,0,0)-IVIRTCORE-ICORE,IALL(0,0,0)-IVIRTCORE-ICORE,ER,VR,EI)
   IROOT2=0
   DO IA=1,IC
    IF (DABS(ER(IA)-OMEGA) < 1.0D-5) IROOT2=IA
   ENDDO
   IF (IROOT2 /= 0) THEN
...
!  WRITE(6,'(A)') "EXACT GREEN'S FUNCTION"
!  CALL DUMP5(GREEN,IALL(0,0,0)-IVIRTCORE-ICORE)
!  WRITE(6,'(A)') "EXACT GREEN'S FUNCTION DERIVATIVE"
!  CALL DUMP5(DGREEN,IALL(0,0,0)-IVIRTCORE-ICORE)
!  WRITE(6,'(A)') "INVERSE OF EXACT GREEN'S FUNCTION"
!  CALL DUMP5(B,IALL(0,0,0)-IVIRTCORE-ICORE)
   RESIDUE=0.0D0
   DO IA=1,IC
    DO IB=1,IC
     RESIDUE=RESIDUE+VR(IA,IROOT2)*DGREEN(IA,IB)*VR(IB,IROOT2)
    ENDDO
   ENDDO 
   RESIDUE=1.0D0/(1.0D0-RESIDUE)
   DO ID=1,IC
    RESIDUES(ID,ID)=0.0D0
    DO IA=1,IC
     DO IB=1,IC
      RESIDUES(ID,ID)=RESIDUES(ID,ID)+VR(IA,ID)*DGREEN(IA,IB)*VR(IB,ID)
     ENDDO
    ENDDO
    RESIDUES(ID,ID)=1.0D0/(1.0D0-RESIDUES(ID,ID))
   ENDDO 
   IF (IROOT <= NCF*IP_NCF) THEN
    DO IA=1,IC
     DO IB=1,IC
      GM=GM+VR(IA,IROOT2)*H(IA+ICORE,IB+ICORE)*VR(IB,IROOT2)*RESIDUE
     ENDDO
    ENDDO
    GM=GM+OMEGA*RESIDUE
    WRITE(6,'(A,I5,3F20.14)') 'IP ROOT',IROOT,OMEGA,ER(IROOT2),RESIDUE
    WRITE(888,'(2F20.14)') OMEGA,RESIDUE
    RESSUM2=RESSUM2+RESIDUE
   ELSE
    WRITE(6,'(A,I5,3F20.14)') 'EA ROOT',IROOT-NCF*IP_NCF,OMEGA,ER(IROOT2),RESIDUE
    WRITE(888,'(2F20.14)') OMEGA,RESIDUE
   ENDIF
   ...
   IF (IROOT <= NCF*IP_NCF) THEN
    WRITE(6,'(A,I5,F20.14,A20,F20.14)') 'IP ROOT',IROOT,OMEGA,'NOT FOUND',0.0D0
    WRITE(888,'(2F20.14)') OMEGA,0.0D0
   ELSE
    WRITE(6,'(A,I5,F20.14,A20,F20.14)') 'EA ROOT',IROOT-NCF*IP_NCF,OMEGA,'NOT FOUND',0.0D0
    WRITE(888,'(2F20.14)') OMEGA,0.0D0
   ENDIF
   ...
   IF (ICORE /= 0) THEN
    CALL WARNING('GALITSKII-MIGDAL FORMULA VIOLATES THE BAYM LAW WHEN FROZEN CORE')
    DO MOP=1,ICORE
      GM=GM+(H(MOQ,MOQ)+EPSILON(MOQ,0,0,0))
    ENDDO
   ENDIF

   GM=GM+NUCLEAR_REPULSION
   WRITE(6,'(A,A25,A20,F20.14)') '       ',' ','RESIDUE SUM',RESSUM
   WRITE(6,'(A,A25,A20,F20.14)') '       ',' ','IP RESIDUE SUM',RESSUM2
   WRITE(6,'(A,A25,A20,F20.14)') '       ',' ','GALITSKII-MIGDAL',GM
   WRITE(6,'(A)') 'fort.888 has exact OMEGA, RESIDUE'

   DEALLOCATE(EIP,WIP,EEA,WEA,VEC0,VEC1,VIP1,VIP2,VIP3,DVIP,VEA1,VEA2,VEA3,DVEA)
   DEALLOCATE(GREEN,IGREEN,DGREEN,DGREEN2,RESIDUES)
   DEALLOCATE(VL,VR,ER,EI,WK)
   DEALLOCATE(A,B,AS,BS,INDX)
