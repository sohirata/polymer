...
!  INCLUDE "mpif.h"
   ...
   DOUBLE PRECISION :: TEMPHF,FMINUS,FPLUS
...
! OLD MPE CODE END
   ...
   ENDIF
   IF (DOPTN(108) > 0.0D0) THEN
    IF (CONVERGE.AND.(MYID == 0)) WRITE(6,'(A,F25.14,A)') 'TOTAL SCF INTERNAL ENERGY   = ',EHFKS,' HARTREE'
    TEMPHF=DOPTN(108)
    IF (CONVERGE.AND.(MYID == 0)) WRITE(6,'(A,F25.14,A)') 'FERMI ENERGY                = ',FERMI,' HARTREE'
    ! - CHEMICAL POTENTIAL x N
    B=-2.0D0*DFLOAT(IOCC)*FERMI
    IF (CONVERGE.AND.(MYID == 0)) WRITE(6,'(A,F25.14,A)') 'CHEMICAL POTENTIAL CONTRIB  = ',B,' HARTREE'
    EHFKS=EHFKS+B
    ! + kB T (F- ln F- + F+ ln F+)
    B=0.0D0
    DO J=ICORE+1,IALL(0,0,0)
     FMINUS=1.0D0/(DEXP((EPSILON(J,0,0,0)-FERMI)/BOLTZMANN/TEMPHF)+1.0D0)
     FPLUS=1.0D0-FMINUS
     IF ((FMINUS > 0.0D0).AND.(FPLUS > 0.0D0)) B=B+2.0D0*(FMINUS*DLOG(FMINUS)+FPLUS*DLOG(FPLUS))*BOLTZMANN*TEMPHF
    ENDDO
    IF (CONVERGE.AND.(MYID == 0)) WRITE(6,'(A,F25.14,A)') 'ENTROPY                     = ',-B/BOLTZMANN/TEMPHF,' kB'
    IF (CONVERGE.AND.(MYID == 0)) WRITE(6,'(A,F25.14,A)') 'ENTROPY CONTRIBUTION        = ',B,' HARTREE'
    EHFKS=EHFKS+B
    IF (CONVERGE.AND.(MYID == 0)) WRITE(6,'(A,F25.14,A)') 'TOTAL SCF GRAND POTENTIAL   = ',EHFKS,' HARTREE'
   ELSE
